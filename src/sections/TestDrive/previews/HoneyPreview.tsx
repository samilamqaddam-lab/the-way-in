  ...
  return (
    <div className="flex min-h-52 flex-col items-center justify-center gap-2.5 bg-[#FFF6E0] p-4 text-center text-ink">
      {stage >= 1 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={popSpring}>
          <p className="font-display text-xl font-extrabold leading-tight text-ink">
            {fr ? 'Le Miel de Rosa 🍯' : "Rosa's Honey 🍯"}
          </p>
          <p className="font-serif text-xs italic text-neutral-500">
            {fr ? 'd’abeilles endormies, avec amour' : 'from sleepy bees, with love'}
          </p>
        </motion.div>
      )}
      {stage >= 2 && (
        ...
        >
          {[
            [fr ? 'Fleurs sauvages' : 'Wildflower', '6€'],
            [fr ? 'Forêt' : 'Forest', '7€'],
            [fr ? 'Printemps' : 'Spring', '6€'],
          ].map(([name, price]) => (
      ...
      {stage >= 3 && (
        ...
        >
          {fr ? '📍 marché du samedi, stand 12' : '📍 Saturday market, stall 12'}
        </motion.span>
      )}
    </div>
  )
