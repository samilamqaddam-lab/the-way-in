    ...
    <div className="flex min-h-52 flex-col items-center justify-center gap-2.5 bg-[#E8F6F1] p-4 text-center">
      {stage >= 1 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={popSpring}>
          ...
          <p className="font-mono text-[0.62rem] text-neutral-500">7 of 12 packed</p>
        </motion.div>
      )}
      {stage >= 2 && (
        <motion.div
        ...
        >
          <p className="mb-1 rounded bg-[#FFF3C4] px-1.5 py-0.5 font-mono text-[0.6rem] font-bold">
            ⚠️ THE CHARGER. ALWAYS THE CHARGER.
          </p>
          <ul className="space-y-1 text-xs text-neutral-800">
            <li className="flex items-center gap-2">
              ...
              <span className="line-through opacity-60">Toothbrush</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="h-4 w-4 rounded border-2 border-ink bg-white" />
              <span>Comfy shoes</span>
            </li>
          </ul>
        </motion.div>
      )}
